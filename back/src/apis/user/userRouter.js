...
userRouter.get(
    ...
    (req, res) => {
        ...
        if (isMember === false || (isMember && user.hasAddtionalInfo === false)) {
            ...
            res.status(302).redirect("http://localhost:3000/useredit"); // 추가정보입력 프론트 페이지 이동
        } else {
            res.status(302).json().redirect("http://localhost:3000"); // 로그인 성공 시 메인 페이지(프론트메인페이지)로 이동(백엔드에서 처리하는게 맞는지?) -> jwt 토큰 응답으로 바꾸기
        }
    },
);
